...
// ```PHP
$options = array(
  'google_maps_review_cid' => '18003678446276593440', // Customer Identification (CID)
  'show_only_if_with_text' => false, // true = show only reviews that have text
  'show_only_if_greater_x' => 4,     // (0-4) only show reviews with more than x stars
...
function getReviews($option) {
  ...
  if (isset($reviews)) {                                                                                                                               /* CHECK REVIEWS */
    if ( isset($option['sort_by_reating_best_1']))                                /* CHECK SORT */
    ...
    if (isset($option['show_rule_after_review']) and $option['show_rule_after_review'] == true) $return .= '<hr class ="google_review_hr" size="1">';                            /* RULER */
