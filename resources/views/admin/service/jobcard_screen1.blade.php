...
$(document).ready(function(){
  ...
  var charges = @php echo json_encode($charges); @endphp;
  $.each(charges, function(key,val) {
      if (val.sub_type == "Parts") {
          $("#part_discount").prop('checked',true)
          $("#part_promo").val(val.promo_code)
      }
      if (val.sub_type == "Service") {
         $("#service_discount").prop('checked',true)
          $("#service_promo").val(val.promo_code)
      }
      if (val.sub_type == "Labour") {
         $("#labour_discount").prop('checked',true)
          $("#labour_promo").val(val.promo_code)
      }
      if (val.sub_type == "Engine Oil") {
           $("#engine_discount").prop('checked',true)
          $("#engine_promo").val(val.promo_code)
      }
      if (val.sub_type == "Pickup & Drop") {
           $("#pickup_discount").prop('checked',true)
          $("#pickup_promo").val(val.promo_code)
      }
  });
   ...
   <div id="app">
         ...
         <div class="alert alert-success" id="js-msg-success" style="display: none;">
            ...
            <div class="box box-primary">
               ...
               <form  action="/admin/service/create/jobcard/screen1/{{$jobCardId}}" method="post" id="form-id">
                    @csrf
                  <div class="row">
                     ...
                     <div class="col-md-6 margin-bottom">
                          ...
                          <input type="hidden" name="amc" id="amc" value="{{$amc}}">
                           ...
                           <select name="job_card_type" id="job_card_type" class="input-css select2">
                              ...
                              @if($val->key == $jobCard->job_card_type)
                                 ...
                                 @else
                                 <option value="{{$val->key}}" {{(old('job_card_type') == $val->key)? 'selected' : '' }} >{{$val->value}}</option>
                              @endif
                              @endforeach
                           </select>
                        ...
                        <div class="col-md-12 margin-bottom">
                          <div class="col-md-6"  id="only_wash"  @if($jobCard['only_wash'] == 'Yes') style="display: block;" @else style="display: none;" @endif >
                             <label for="only_wash">Only Wash</label>
                           <input type="radio" name="only_wash" value="Yes" @if($jobCard['only_wash'] == 'Yes') checked="" @endif> Yes
                          </div>

                           <div class="col-md-6"  id="mejor_job"  @if($jobCard['mejor_job'] == 'Yes') style="display: block;" @else style="display: none;" @endif >
                             <label for="mejor_job">Mejor Job</label>
                           <input type="checkbox" name="mejor_job" value="Yes" @if($jobCard['mejor_job'] == 'Yes') checked @endif> Yes
                          </div> 
  
                        </div>
                        <input type="hidden" name="vehicle_type" id="vehicle_type" value="{{$jobCard->vehicle_type}}">
                          <div class="col-md-12 margin-bottom" id="general_sub_type" @if($jobCard->job_card_type == 'General') style="display: block;" @else style="display: none;" @endif>
                             <label for="sub_type">Sub Type</label>
                              <select name="sub_type[]" id="sub_type" class="form-control select2" multiple="multiple" style="width: 100%;">
                                @php $subtype = explode(',',$jobCard->sub_type) @endphp
                                  @foreach($general_subtype as $sub_type)
                                  @if(in_array($sub_type->id, $subtype))
                                  <option value="{{$sub_type->id}}" selected >{{$sub_type->name}}</option>
                                  @else
                                  <option value="{{$sub_type->id}}" {{old('sub_type')==$sub_type->id ? 'selected=selected' : ''}}>{{$sub_type->name}}</option>
                                  @endif
                                  @endforeach
                              </select>
                            </div>

                            <div class="col-md-12 margin-bottom" id="Old_JC" @if(in_array(19, $subtype))  style="display: block;" @else  style="display: none;" @endif>
                              ...
                              <select class="select2" name="old_job_card" id="old_jobcard" style="width: 100%;">
                                ...
                                @if($jobCard->old_jobcard == $item->id))
                            ...
                            <div class="col-md-12 margin-bottom" id="free_JC" @if($jobCard->job_card_type == 'Free') style="display: block;" @else style="display: none;"@endif>
                              ...
                              <select class="input-css select2" name="free_sub_type" id="free_sub_type" style="width: 100%;">
                                <option selected="" disabled="">Select sub type</option>
                                 @foreach($free_subtype as $sub_type)
                                  @if($sub_type->id == $jobCard->sub_type)
                                  <option value="{{$sub_type->id}}" selected>{{$sub_type->name}}</option>
                                  @else
                                  <option value="{{$sub_type->id}}" {{old('free_sub_type')==$sub_type->id ? 'selected=selected' : ''}}>{{$sub_type->name}}</option>
                                  @endif
                                  @endforeach
                              </select>
                            </div>
                            ...
                            <div class="col-md-12 margin-bottom" id="acc_JC" style="display: none;">
                              ...
                              <select class="input-css select2" name="acc_sub_type" id="acc_sub_type" style="width: 100%;">
                                <option selected="" disabled="">Select sub type</option>
                                @foreach($acc_subtype as $sub_type)
                                   @if($sub_type->id == $jobCard->sub_type)
                                  <option value="{{$sub_type->id}}" selected>{{$sub_type->name}}</option>
                                  @else
                                  <option value="{{$sub_type->id}}" {{old('acc_sub_type')==$sub_type->id ? 'selected=selected' : ''}}>{{$sub_type->name}}</option>
                                  @endif
                                  @endforeach
                              </select>
                            </div>
                           ...
                           <div class="col-md-12 margin-bottom">
                              ...
                              <input type="text" name="mobile" id="mobile" value="{{(old('mobile'))?old('mobile') : $mobile}}" class="input-css" placeholder="Enter Mobile Number">
                           ...
                           <div class="col-md-12 margin-bottom">
                              ...
                              <input type="text" name="frame_km" id="frame_km" value="{{old('frame_km')? old('frame_km'):$jobCard->vehicle_km}}" class="input-css" placeholder="Enter Vehicle KiloMeter">
                              {!! $errors->first('frame_km', '<p class="help-block">:message</p>') !!}
                           </div>
                           <div id="duration_div" style="display: block;">

                           <div class="col-md-12 margin-bottom">
                              <label for="oilinfornt_customer">Insurance available ?<sup>*</sup></label>
                              {!! $errors->first('insurance', '
                              <p class="help-block">:message</p>
                              ') !!}
                              <div class="col-md-2">
                                 <div class="radio">
                                    <label><input autocomplete="off" @if(isset($getInsurance->id) || $jobCard->insurance == 'Yes') checked  @endif type="radio" class="" value="Yes" name="insurance">Yes</label>
                                 </div>
                              </div>
                              <div class="col-md-2">
                                 <div class="radio">
                                    <label><input autocomplete="off"  @if( $jobCard->insurance == 'No') checked  @endif  {{(old('insurance') == 'No') ? 'checked' : '' }}   type="radio" class="" value="No" name="insurance">No</label>
                                 </div>
                              </div>
                           </div>


                           <div class="col-md-12 margin-bottom" id="renewal-div" @if(isset($renewl_date) || $jobCard->insurance == 'Yes') style="display: block;"  @else style="display: none;" @endif >
                           ...
                           <div class="col-md-12 margin-bottom" id="available-div" @if($jobCard->insurance == 'No') style="display: block;" @else style="display: none;" @endif>
                              ...
                              <select class="input-css select2" name="available_type" style="width: 100%;">
                                ...
                                <option value="Doc Not Available" @if($jobCard->insurance_available_type == 'Doc Not Available') selected @endif >Doc Not Available</option>
                                <option value="Insurance Not Available" @if($jobCard->insurance_available_type == 'Insurance Not Available') selected @endif >Insurance Not Available</option>
                              </select>
                           </div>
                         </div>
                      ...
                      <div class="col-md-6 margin-bottom" id="hjc_discount" style="display: none;">
                        ...
                        <div class="row">
                        <div class="col-md-3">
                          <input type="checkbox" name="part_discount" id="part_discount" value="Parts"> Parts
                           </div>
                           <div class="col-md-9">
                            <input type="text" name="part_promo" id="part_promo" class="input-css" placeholder="Enter Promo Code">  
                          </div>
                         </div><br>
                         <div class="row">
                        <div class="col-md-3">
                          <input type="checkbox" name="service_discount" id="service_discount" value="Service"> Service
                           </div>
                           <div class="col-md-9">
                            <input type="text" name="service_promo" id="service_promo" class="input-css" placeholder="Enter Promo Code">  
                          </div>
                         </div><br>
                        <div class="row">
                        <div class="col-md-3">
                          <input type="checkbox" name="labour_discount" id="labour_discount" value="Labour"> Labour
                           </div>
                           <div class="col-md-9">
                            <input type="text" name="labour_promo" id="labour_promo" class="input-css" placeholder="Enter Promo Code">  
                          </div>
                         </div><br>
                          <div class="row">
                        <div class="col-md-3">
                          <input type="checkbox" name="engine_discount" id="engine_discount" value="Engine Oil"> Engine Oil
                           </div>
                           <div class="col-md-9">
                            <input type="text" name="engine_promo" id="engine_promo" class="input-css" placeholder="Enter Promo Code">  
                          </div>
                         </div><br>
                          <div class="row">
                        <div class="col-md-3">
                          <input type="checkbox" name="pickup_discount" id="pickup_discount" value="Pickup & Drop"> Pickup & Drop
                           </div>
                           <div class="col-md-9">
                            <input type="text" name="pickup_promo" id="pickup_promo" class="input-css" placeholder="Enter Promo Code">  
                          </div>
                           {!! $errors->first('job_card_type', '<p class="help-block">:message</p>') !!}
                         </div>
                      </div>
